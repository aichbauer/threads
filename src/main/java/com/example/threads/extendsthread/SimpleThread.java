...
public class SimpleThread {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i <= 100; i++) {
            // because we are extending our Task with the Thread class
            // we just need to instantiate the SimpleTask
            // simple task now has access to all methods of the Thread class
            // so we can start the Thread directly on our class
            // we can also write that as
            // Thread thread = new SimpleTask();
            SimpleTask thread = new SimpleTask();
